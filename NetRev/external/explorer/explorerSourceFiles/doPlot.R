  ...
  if(PermitPlot()){
    ...
    if(type!="summary"){
      ...
      if(max(dat$AK_FLAG, na.rm=T)==0){#input$CategorySelect == "Homeport" & 
        g <- g + labs(y = paste("Thousands ($)", "(",input$StatSelect, ")"), x="  Vessels are grouped into three tiered categories: top, middle, and lower earners based on revenue. This is done for each year separately.", title = main()) 
      } else {
        g <- g + labs(y = paste("Thousands ($)", "(",input$StatSelect, ")"), x=" Vessels are grouped into three tiered categories: top, middle, and lower earners based on revenue. This is done for each year separately.  
