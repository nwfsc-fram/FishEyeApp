...
opts_chunk$set(results = 'asis', concordance = F, echo = F, cache = F, 
...
vesselhomeportdata$PORT <- ifelse(vesselhomeportdata$PORT=="Morro Bay", "Monterey", as.character(vesselhomeportdata$PORT))
...
portlenakSUM <- as.data.frame(dat %>% group_by(VESSEL_ID, STATE,FSHAK,HOMEPT,VSSLNGCLASS, YEAR) %>% summarise(Fixed_costs=sum(Fixed_costs), Revenue=sum(Revenue), Total_cost_net_revenue=sum(Total_cost_net_revenue), Variable_cost_net_revenue=sum(Variable_cost_net_revenue), Variable_costs=sum(Variable_costs)))
...
portlenaklong<-melt(portlenakSUM[, !names(portlenakSUM) %in% c('MTS', 'DAS','FEWERFISHERIES','HOMEPT','STATE')], id.vars=c('VESSEL_ID', 'YEAR', 'FSHAK','VSSLNGCLASS'))
colnames(portlenaklong)[5]<-'SUMSTAT'
## Check confidentiality issues
trt1 <- PreTreat(portlenaklong, variables=.(VSSLNGCLASS, YEAR,SUMSTAT, FSHAK), valvar="value",individual="VESSEL_ID")
trt2 <- PreTreat(portlenaklong, variables=.(VSSLNGCLASS, YEAR,SUMSTAT), valvar="value",individual="VESSEL_ID")
portlenaklong.trt <-  cbind(trt1[,-which(colnames(trt1)=="aggvals")], trt2[,which(colnames(trt2)=="finalnum")])
colnames(portlenaklong.trt)[7]="finalnum2" 
portlenaklong.trt <- merge(portlenaklong, portlenaklong.trt, by=c("VESSEL_ID","VSSLNGCLASS","YEAR","SUMSTAT", "FSHAK"))
portlenaklong.trt$value <- with(portlenaklong.trt, ifelse(FSHAK=="FALSE", finalnum, finalnum2))

VSSLNGall<-rbind(data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+VSSLNGCLASS~., 'value', mean, 'value'), FISHAK='TRUE', SUMCAT='Vessel length class', variable='Average'),
                 data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+VSSLNGCLASS~., 'value', mean, 'value'), FISHAK='FALSE', SUMCAT='Vessel length class', variable='Average'),
                 data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+VSSLNGCLASS~., 'value', sum, 'value'), FISHAK='TRUE', SUMCAT='Vessel length class', variable='Total'),
                 data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+VSSLNGCLASS~., 'value', sum, 'value'), FISHAK='FALSE', SUMCAT='Vessel length class', variable='Total'), 
                 data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+VSSLNGCLASS~., 'value', length, 'value'), FISHAK='TRUE', SUMCAT='Vessel length class', variable='N'),
                 data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+VSSLNGCLASS~., 'value', length, 'value'), FISHAK='FALSE', SUMCAT='Vessel length class', variable='N'))
VSSLNGall$variable<-with(VSSLNGall, paste(variable, SUMSTAT, sep=" "))
VSSLNGall<-VSSLNGall[-2]
colnames(VSSLNGall)[2]<-'SUMVAR'

portlenaklong<-melt(portlenakSUM[, !names(portlenakSUM) %in% c('MTS', 'DAS','FEWERFISHERIES','VSSLNGCLASS','STATE')], id.vars=c('VESSEL_ID', 'YEAR', 'FSHAK','HOMEPT'))
colnames(portlenaklong)[5]<-'SUMSTAT'
## Check confidentiality issues
trt1 <- PreTreat(portlenaklong, variables=.(HOMEPT, YEAR,SUMSTAT, FSHAK), valvar="value",individual="VESSEL_ID")
trt2 <- PreTreat(portlenaklong, variables=.(HOMEPT, YEAR,SUMSTAT), valvar="value",individual="VESSEL_ID")
portlenaklong.trt <-  cbind(trt1[,-which(colnames(trt1)=="aggvals")], trt2[,which(colnames(trt2)=="finalnum")])
colnames(portlenaklong.trt)[7]="finalnum2" 
portlenaklong.trt <- merge(portlenaklong, portlenaklong.trt, by=c("VESSEL_ID","HOMEPT","YEAR","SUMSTAT", "FSHAK"))
portlenaklong.trt$value <- with(portlenaklong.trt, ifelse(FSHAK=="FALSE", finalnum, finalnum2))

HOMEPTall<-rbind(data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+HOMEPT~., 'value', mean, 'value'), FISHAK='TRUE', SUMCAT='Homeport', variable='Average'),
                 data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+HOMEPT~., 'value', mean, 'value'), FISHAK='FALSE', SUMCAT='Homeport', variable='Average'),
                 data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+HOMEPT~., 'value', sum, 'value'), FISHAK='TRUE', SUMCAT='Homeport', variable='Total'),
                 data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+HOMEPT~., 'value', sum, 'value'), FISHAK='FALSE', SUMCAT='Homeport', variable='Total'), 
                 data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+HOMEPT~., 'value', length, 'value'), FISHAK='TRUE', SUMCAT='Homeport', variable='N'),
                 data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+HOMEPT~., 'value', length, 'value'), FISHAK='FALSE', SUMCAT='Homeport', variable='N'))
HOMEPTall$variable<-with(HOMEPTall, paste(variable, SUMSTAT, sep=" "))
HOMEPTall<-HOMEPTall[-2]
colnames(HOMEPTall)[2]<-'SUMVAR'

portlenaklong<-melt(portlenakSUM[, !names(portlenakSUM) %in% c('MTS', 'DAS','FEWERFISHERIES','VSSLNGCLASS','HOMEPT')], id.vars=c('VESSEL_ID', 'YEAR', 'FSHAK','STATE'))
colnames(portlenaklong)[5]<-'SUMSTAT'
## Check confidentiality issues
trt1 <- PreTreat(portlenaklong, variables=.(STATE, YEAR,SUMSTAT, FSHAK), valvar="value",individual="VESSEL_ID")
trt2 <- PreTreat(portlenaklong, variables=.(STATE, YEAR,SUMSTAT), valvar="value",individual="VESSEL_ID")
portlenaklong.trt <-  cbind(trt1[,-which(colnames(trt1)=="aggvals")], trt2[,which(colnames(trt2)=="finalnum")])
colnames(portlenaklong.trt)[7]="finalnum2" 
portlenaklong.trt <- unique(merge(portlenaklong, portlenaklong.trt, by=c("VESSEL_ID","STATE","YEAR","SUMSTAT", "FSHAK")))
portlenaklong.trt$value <- with(portlenaklong.trt, ifelse(FSHAK=="FALSE", finalnum, finalnum2))

STATEall<-rbind(data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+STATE~., 'value', mean, 'value'), FISHAK='TRUE', SUMCAT='State', variable='Average'),
                data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+STATE~., 'value', mean, 'value'), FISHAK='FALSE', SUMCAT='State', variable='Average'),
                data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+STATE~., 'value', sum, 'value'), FISHAK='TRUE', SUMCAT='State', variable='Total'),
                data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+STATE~., 'value', sum, 'value'), FISHAK='FALSE', SUMCAT='State', variable='Total'), 
                data.frame(DotDcast(portlenaklong.trt, YEAR+SUMSTAT+STATE~., 'value', length, 'value'), FISHAK='TRUE', SUMCAT='State', variable='N'),
                data.frame(DotDcast(portlenaklong.trt[portlenaklong.trt$FSHAK=='FALSE',], YEAR+SUMSTAT+STATE~., 'value', length, 'value'), FISHAK='FALSE', SUMCAT='State', variable='N'))
...
trt1<-(PreTreat(fishrates, variables=.(VSSLNGCLASS, YEAR, FSHAK, variable), valvar="value",individual="VESSEL_ID"))
trt2<-(PreTreat(fishrates, variables=.(VSSLNGCLASS, YEAR, variable), valvar="value",individual="VESSEL_ID"))
fishrates.trt <-  cbind(trt1[,-which(colnames(trt1)=="aggvals")], trt2[,which(colnames(trt2)=="finalnum")])
colnames(fishrates.trt)[7]="finalnum2" 
fishrates.trt <- merge(fishrates, fishrates.trt, by=c("VESSEL_ID","VSSLNGCLASS","YEAR","variable", "FSHAK"))
fishrates.trt$value <- with(fishrates.trt, ifelse(FSHAK=="FALSE", finalnum, finalnum2))
fishrates.trt <- unique(fishrates.trt[,-which(colnames(fishrates.trt)==c("finalnum","finalnum2"))])
VSSLNGrates<-rbind(data.frame(DotDcast(fishrates.trt, YEAR+variable+VSSLNGCLASS~., 'value', mean, 'value'), FISHAK='TRUE', SUMCAT='Vessel length class'),
                   data.frame(DotDcast(fishrates.trt[fishrates.trt$FSHAK=='FALSE',], YEAR+variable+VSSLNGCLASS~., 'value', mean, 'value'), FISHAK='FALSE', SUMCAT='Vessel length class'))
colnames(VSSLNGrates)[3]<-'SUMVAR'

trt1<-(PreTreat(fishrates, variables=.(HOMEPT, YEAR, FSHAK, variable), valvar="value",individual="VESSEL_ID"))
trt2<-(PreTreat(fishrates, variables=.(HOMEPT, YEAR, variable), valvar="value",individual="VESSEL_ID"))
fishrates.trt <-  cbind(trt1[,-which(colnames(trt1)=="aggvals")], trt2[,which(colnames(trt2)=="finalnum")])
colnames(fishrates.trt)[7]="finalnum2" 
fishrates.trt <- merge(fishrates, fishrates.trt, by=c("VESSEL_ID","HOMEPT","YEAR","variable", "FSHAK"))
fishrates.trt$value <- with(fishrates.trt, ifelse(FSHAK=="FALSE", finalnum, finalnum2))
fishrates.trt <- unique(fishrates.trt[,-which(colnames(fishrates.trt)==c("finalnum","finalnum2"))])
HOMEPTrates<-rbind(data.frame(DotDcast(fishrates.trt, YEAR+variable+HOMEPT~., 'value', mean, 'value'), FISHAK='TRUE', SUMCAT='Homeport'),
                   data.frame(DotDcast(fishrates.trt[fishrates.trt$FSHAK=='FALSE',], YEAR+variable+HOMEPT~., 'value', mean, 'value'), FISHAK='FALSE', SUMCAT='Homeport'))
colnames(HOMEPTrates)[3]<-'SUMVAR'

trt1<-(PreTreat(fishrates, variables=.(STATE, YEAR, FSHAK, variable), valvar="value",individual="VESSEL_ID"))
trt2<-(PreTreat(fishrates, variables=.(STATE, YEAR, variable), valvar="value",individual="VESSEL_ID"))
fishrates.trt <-  cbind(trt1[,-which(colnames(trt1)=="aggvals")], trt2[,which(colnames(trt2)=="finalnum")])
colnames(fishrates.trt)[7]="finalnum2" 
fishrates.trt <- merge(fishrates, fishrates.trt, by=c("VESSEL_ID","STATE","YEAR","variable", "FSHAK"))
fishrates.trt$value <- with(fishrates.trt, ifelse(FSHAK=="FALSE", finalnum, finalnum2))
fishrates.trt <- unique(fishrates.trt[,-which(colnames(fishrates.trt)==c("finalnum","finalnum2"))])
STATErates<-rbind(data.frame(DotDcast(fishrates.trt, YEAR+variable+STATE~., 'value', mean, 'value'), FISHAK='TRUE', SUMCAT='State'),
                  data.frame(DotDcast(fishrates.trt[fishrates.trt$FSHAK=='FALSE',], YEAR+variable+STATE~., 'value', mean, 'value'), FISHAK='FALSE', SUMCAT='State'))
colnames(STATErates)[3]<-'SUMVAR'

full.dat<-rbind(fisheriesall, fisheriesrates[,c(1,3:6,2)], VSSLNGrates[,c(1,3:6,2)], HOMEPTrates[,c(1,3:6,2)], STATErates[,c(1,3:6,2)], VSSLNGall, HOMEPTall, STATEall)
colnames(full.dat)=c("YEAR","VARIABLE","VALUE","FISHAK","CATEGORY","STAT_DESCR")
###
...
full.dat <- within(full.dat, var <- data.frame(do.call('rbind', strsplit(full.dat$STAT_DESCR, '(?<=[ ])(?=[A-Z])', perl=T))))
full.dat$SHORTDESC <- full.dat$var$X1
full.dat$STAT <- full.dat$var$X2
full.dat <- full.dat[,-c(6:7)]
# rename category --------------------------------------------------------------
...
size <- subset(full.dat %>% group_by(YEAR,VARIABLE,FISHAK,CATEGORY,SHORTDESC,STAT) %>% transmute(N=max(VALUE)), STAT=="N")
full.dat <- merge(full.dat, size[,-6], by=c("YEAR","VARIABLE","FISHAK","CATEGORY","SHORTDESC"))
full.dat <- subset(full.dat, STAT!="N")
full.dat$VALUE <- ifelse(full.dat$N<3, NA, full.dat$VALUE)
                  
########
save(full.dat, file="./netrevTable.Rdata")
