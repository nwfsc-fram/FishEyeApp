tags$div(style = "margin: 15px 15px 30px; width: 60%",    
   ...
   h4("Summary Variables"),
     tags$ul(
       tags$li(strong("Fisheries:"),'The Net Revenue Explorer uses the same fisheries definitions as those used by the', 
                ...
                tags$a(href="http://www.westcoast.fisheries.noaa.gov/fisheries/groundfish_catch_shares/", 'The West Coast Groundfish Trawl Catch Share Program', target="_blank"), 
       ...
       tags$li(strong("Vessel length class:"),"Three categories of vessel length representing the range of catcher vessel length. 
                The categories are large vessels (> 80 ft), medium vessels (> 60 ft, <= 80 ft), and small vessels (< 60 ft)."),
       tags$li(strong("Homeport:"), "The", tags$a(href="map.pdf", "homeport", target="_blank"), "reported by each vessel on the", tags$a(href="http://www.nwfsc.noaa.gov/research/divisions/fram/economic/economic_data_forms.cfm", "EDC survey form", target="_blank"), "aggregated to point groups."), 
  ...
  tags$ul(
    ...
    tags$li(strong("Fleet-wide total:"),"Total for all vessels that participated in the Catch Share program.") ),tags$br(),
    ...
    ,tags$br(),
      ...
      tags$ul(
        ...
        tags$li(strong("Variable cost:"),
          "Variable costs vary with the level of participation
        ...
        tags$li(strong("Fixed cost:"),
           "Fixed costs do not vary with the level of fishing participation or do not vary as directly as variable costs with the level of participation during a given year. 
        ...
        tags$li(strong("Variable cost net revenue (VCNR):"),
           "VCNR is revenue minus variable costs (costs that vary with the level of participation, such as crew, fuel, buyback fees, observers, and other variable costs). VCNR is a measure of the operating profit of the average vessel."
             ),
        tags$li(strong("Total cost net revenue (TCNR):"),
          'TCNR is revenue minus variable and fixed costs (costs that do not vary with the level of participation, such as equipment and gear purchases). TCNR is a longer-term measure of profitability.
...
h4("Fished for whiting"),
tags$ul(
  tags$p("Vessels that also fished for Pacific whiting (versus vessels that did not fish for Pacific whiting) can be filtered from the data. Vessels that fished for whiting tend to be larger and catch a much higher volume of fish.
         In addition, total allowable catch for Pacific whiting has much more annual variation than total annual catch for species targeted in the non-whiting groundfish sector."), 
  tags$p("If this box", tags$strong('is not selected,'), "then only vessels that did not fish for Pacific whiting will be included. If this box", tags$strong('is selected,'), 
         ...
         "Please note that there are some cases where there are not enough observations of vessels that either 1) did not fish for Pacific whiting or
      2) also fished for Pacific whiting. When this occurs, we show results for both groups combined, regardless of whether or not you selected the", tags$em("Include vessels that fished for whiting"), "button.")
), tags$br(),

  h4("Variables in the Data Table"),
 tags$ul(
   tags$li(strong('Year:'), 'Calendar year over which fishing activities occurred.'),
   tags$li(strong('Summary variables:'),  'Summary variables (fisheries, homeport, state, vessel length class) are used to group vessels to better understand the economic performance of particular segments of the fleet. See above for a description of the summary 
          variables.'),
   ...
   tags$li(strong('Thirds:'), 'This variable is only relevant for the', tags$em('Fleet-wide Variability Analysis.'), 'Vessels are grouped into three tiered categories based on revenue: top, middle, and lower revenue earners. This is done for each year separately.'),
   tags$p(tags$br(),
          tags$br(),
          tags$br(),
          tags$br())
