...
output$SectorSelect <- renderUI({
  ...
  tags$div(class = "sectselect", selectInput(
    "Sect_sel",
    span("Select a sector:", style = "font-style:strong;display:inline-block;vertical-align:middle"),
    c(
      'Catcher Vessels' = "CV",
...
output$Yearselect <- renderUI({
  if (input$Ind_sel == "Vessel characteristics") {
    if (input$Sect_sel == "CV" &
        ...
        input$demSelect %in% c('Number of vessels', 'Vessel length', 'Gini coefficient')) {
      tags$div(
        class = "ckbox",
        sliderInput(
          "YearSelect",
    ...
    if (input$Sect_sel == "CV" &
        input$CategorySelect == 'Fisheries' &
        input$socSelect %in% c('Seasonality', 'Share of landings by state')) {
      tags$div(
        class = "ckbox",
        sliderInput(
          "YearSelect",
...
output$IndicatorSelect <- renderUI({
  if (input$Sect_sel != 'FR') {
    selectInput(
      ...
      c('Vessel characteristics', "Economic", "Crew", "Other"),
    ...
    selectInput(
      ...
      c('Vessel characteristics', "Economic", "Other"),
      selected = 'Vessel characteristics',
      selectize = T
      )
  }
      ...
      if (input$CategorySelect == "State") {
        if (input$LayoutSelect != 'Metrics') {
          ...
          if (input$Ind_sel == 'Vessel characteristics' ||
            ...
            tagList(
              tags$div(
                class = "select",
                selectInput("inSelect", "", fishgrps4cats),
                style = "margin-bottom:-10px"
              ),
              checkboxGroupInput(
                "VariableSelect",
                "Select one or more state:",
          ...
          tagList(
            ...
            tags$div(
              ...
              radioButtons(
                "VariableSelect",
                "Select one state:",
                choices = c("No state selected" = "", factorOrder$state),
                selected = ""
              )
            )
          )
        ...
        if (input$LayoutSelect != 'Metrics') {
          if (input$Ind_sel == "Vessel characteristics" ||
            ...
            tagList(
              ...
              checkboxGroupInput(
                "VariableSelect",
                "Select one vessel length class:",
                choices = c(
                  "Large vessel (> 80 ft)",
          ...
          if (input$Ind_sel == "Vessel characteristics" ||
            ...
            tagList(
              ...
              tags$div(
                checkboxGroupInput(
                  "VariableSelect",
                  div("Select one or more homeport:", style = "margin-top:0; padding:-10px"),
            ...
            tagList(
              ...
              tags$div(
                radioButtons(
                  "VariableSelect",
                  div("Select one homeport:", style = "margin-top:0; padding:-10px"),
                  choices = factorOrder$port,
                  selected = ""
                )
              )
          ...
          tagList(
            ...
            tags$div(
              ...
              radioButtons(
                "VariableSelect",
                div("Select one homeport:", style = "margin-top:0; padding:-10px"),
                choices = c("No homeport selected" =
                    "", factorOrder$port),
                selected = ""
              )
            )
        ...
        if (input$LayoutSelect != 'Metrics') {
          if (input$Ind_sel == "Economic" ||
              ...
              input$Ind_sel == 'Crew' ||
              input$Ind_sel == "Other" &
              input$socSelect[1] != "Share of landings by state") {
            if (input$tabs == 'Panel1') {
              tags$div(class = "ckbox2",
                checkboxGroupInput(
                  "VariableSelect",
                  div("Select one or more fisheries:", style = "margin-top:0; padding:-10px"),
          ...
          if (input$Ind_sel == "Vessel characteristics") {
            if (input$demSelect != "Proportion of revenue from catch share species") {
              tagList(
                ...
                tags$div(
                  ...
                  checkboxGroupInput(
                    "VariableSelect",
                    HTML(
                      "<div> Select one or more region:<button id='FRr' type='button' class='btn btn-default action-button shiny-bound-input'>
                      <i class='fa fa-info-circle fa-fw'></i></button></div>"
                    ),
              ...
              tagList(
                tags$div(
                  class = "select",
                  selectInput("inSelect", "", c("All production")),
                  style = "margin-bottom:-10px"
                ),
                tags$div(
                  class = "ckbox",
            ...
            tagList(
              ...
              tags$div(
                ...
                radioButtons(
                  "VariableSelect",
                  HTML(
                    "<div> Select one region:<button id='FRr' type='button' class='btn btn-default action-button shiny-bound-input'>
                    <i class='fa fa-info-circle fa-fw'></i></button></div>"
                  ),
                  choices = c(
                    "No region selected" = "",
                ...
                tagList(
                  ...
                  tags$div(
                    checkboxGroupInput(
                      "VariableSelect",
                      HTML(
                        "<div> Select one size class:<button id='FRs' type='button' class='btn btn-default action-button shiny-bound-input'>
                        <i class='fa fa-info-circle fa-fw'></i></button></div>"
                      ),
          ...
          if (input$Ind_sel == "Vessel characteristics" |
            ...
            tags$div(
              ...
              checkboxGroupInput(
                "VariableSelect",
                HTML(
                  "<div> Select one or more production activities:<button id='FRi' type='button' class='btn btn-default action-button shiny-bound-input'>
            ...
            tags$div(
              ...
              checkboxGroupInput(
                "VariableSelect",
                HTML(
                  "<div> Select one production activity:<button id='FRi' type='button' class='btn btn-default action-button shiny-bound-input'>
                  <i class='fa fa-info-circle fa-fw'></i></button> <style margin-top:0; padding:-10px> </style></div>"
                ),
                #HTML("<div style='font-style:italic; font-size:10.87pt; font-weight:normal; margin-top:8.5pt'>
                choices = c(
          ...
          if (input$Ind_sel == "Other") {
            tags$div(class = 'StatGrey4',
              radioButtons(
                ...
                choices = c(
                  "No production activities selected" = "",
                  "All production",
                  "Groundfish production",
                  "Pacific whiting production",
                  'Non-whiting groundfish production',
...
output$FishAkselect <- renderUI({
  if (input$Sect_sel == 'CV') {
    tags$div(class = "ckbox", checkboxInput(
      "FishAkSelect",
      p(
        "Include Alaskan fisheries activities: ",
        span(
          "By selecting this, you will include data from their activities in Alaska.",
  ...
  if (input$LayoutSelect == 'Metrics') {
    tags$div(
      ...
      checkboxGroupInput(
        "ShortdescrSelect",
        HTML(
          "<div> Economic measures:<button id='iem' type='button' class='btn btn-default action-button shiny-bound-input'> <i class='fa fa-info-circle fa-fw' ></i>
          </button></div>"
        ),
  ...
  if (input$Sect_sel == "FR")  {
    tagList(
      ...
      tags$div(class = "statbox", radioButtons(
        "StatSelect", "",  choices = c(DatVars()$STAT[3:4])
      ))
    )
  } else {
    tagList(
      ...
      tags$div(
        ...
        radioButtons(
          "StatSelect",
          "",
          choices = c(DatVars()$STAT[4:6]),
          selected = DatVars()$STAT[4]
        )
      )
    )
  }
...
output$Layoutselect <- renderUI({
  tags$div(
    radioButtons(
      "LayoutSelect",
      HTML(
        "<div> Compare: <button id='icompare' type='button' class='btn btn-default action-button shiny-bound-input'>
        <i class='fa fa-info-circle fa-fw'></i></button></div>"
      ),
      if (input$Sect_sel == 'CV') {
        choices = c('Groups of Catcher Vessels', 'Metrics')
      }
      else if (input$Sect_sel == 'M') {
        choices = c('Groups of Motherships', 'Metrics')
      }
      else if (input$Sect_sel == 'CP') {
        choices = c('Groups of Vessels', 'Metrics')
      }
      else if (input$Sect_sel == 'FR') {
        choices = c('Groups of processors', 'Metrics')
      }
      ,
      selected = choices[1],
      inline = T
...
output$Plotselect <- renderUI({
  tags$div(class = "ckbox", checkboxInput(
    "PlotSelect",
    HTML(
      "<div> <span' style='font-weight:bold;font-size:12pt'>Show variance: </span>
